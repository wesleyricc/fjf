...
class _FantasyHomeScreenState extends State<FantasyHomeScreen> {
  ...
  // --- DETALHAMENTO SCOUT COM PONTOS POR ESTATÍSTICA ---
  ...
  Widget _buildScoutIcon(IconData icon, String label, int value, Color color, double points) {
    return Column(
      children: [
        ...
        // 🚨 EXIBIÇÃO DOS PONTOS EQUIVALENTES 🚨
        Text(
          "${points > 0 ? '+' : ''}${points.toStringAsFixed(1)}", 
          style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: points == 0 ? Colors.grey : (points > 0 ? Colors.green : Colors.red))
        ),
      ],
    );
  }
                        ...
                        vm.isMarketOpen 
                            ...
                            : () {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text("Mercado Fechado! Escalações bloqueadas."))
                                );
                              },
                      ),
                ...
                child: Row(
                  ...
                  children: [
                    const Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("TOTAL DA PARCIAL", style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1)),
                        Text("Rodada Atual", style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w500)),
                      ],
                    ),
                    Text(
                      "${vm.teamPartialScore.toStringAsFixed(2)} pts", 
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Text(player.position.substring(0, 1), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.grey, fontSize: 12)),
              ),
              ...
              if (!isMarketOpen)
                Container(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(fontWeight: FontWeight.w900, color: scoreColor, fontSize: 14)
                  ),
                ),
            ],
          ),
  ...
  Widget _buildLoginView(BuildContext context, FantasyAuthService authService) { 
    return Scaffold( 
      body: Center( 
        child: ElevatedButton(onPressed: () => authService.signInWithGoogle(), child: const Text("Entrar com Google"))
      )
    ); 
  }
  ...
  Widget _buildTeamHeader(BuildContext context, FantasyTeam team, bool isMarketOpen, int round) { 
    return Container( 
      ...
      decoration: BoxDecoration(color: Theme.of(context).primaryColor, borderRadius: const BorderRadius.vertical(bottom: Radius.circular(30))), 
      child: Row(
        children: [
          CircleAvatar(
            radius: 30, 
          ...
          Column(
            crossAxisAlignment: CrossAxisAlignment.start, 
            children: [
              Text(team.teamName, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)), 
              Text("Rodada $round - ${isMarketOpen ? 'ABERTO' : 'FECHADO'}", style: const TextStyle(color: Colors.white70))
            ]
          )
        ]
      )

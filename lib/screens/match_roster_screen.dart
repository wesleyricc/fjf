...
import '../widgets/player_display_card.dart'; // Importa o widget atualizado
import '../services/admin_service.dart';
import '../widgets/sponsor_banner_rotator.dart';
import 'package:intl/intl.dart';

// --- Classe Wrapper (Mock) (Sem mudanças) ---
class FakeDocumentSnapshot implements DocumentSnapshot {
  ...
  FakeDocumentSnapshot({
    ...
    required Map<String, dynamic> dataMap,
       ...
       _data = dataMap,
       reference = FirebaseFirestore.instance.collection('players').doc(id);
  ...
  @override
  get(Object field) => _data[field];
  @override
  operator [](Object field) => _data[field];
  @override
  SnapshotMetadata get metadata => throw UnimplementedError();
}
// --- Fim da Classe Wrapper ---
...
class _MatchRosterScreenState extends State<MatchRosterScreen> {
  ...
  // Estados dos jogadores (como antes)
  ...
  // --- NOVO: ESTADOS PARA A COR DA EQUIPE ---
  Color? _team1Color;
  Color? _team2Color;
  // --- FIM DA MUDANÇA ---
  ...
  // --- NOVO: HELPER PARA CONVERTER HEX STRING EM COR ---
  Color? _parseColor(String? hexColor) {
    if (hexColor == null || hexColor.isEmpty) {
      return null;
    }
    try {
      // Remove o '#' (se existir) e adiciona o 'ff' (alpha)
  ...
  // --- ALTERAÇÃO: FUNÇÃO AGORA BUSCA A COR DA EQUIPE ---
  Future<void> _fetchAndProcessRosters() async {
    ...
    try {
      // 1. Busca os jogadores de ambos os times (como antes)
      ...
      // 2. Busca os DOCUMENTOS DOS TIMES para ler 'default_starters' e 'team_color'
      ...
      // 3. Pega a lista de IDs dos titulares padrão
      List<String> startersHomeIds = List<String>.from(
        team1Data['default_starters'] ?? [],
      );
      List<String> startersAwayIds = List<String>.from(
        team2Data['default_starters'] ?? [],
      );

      // 4. Pega as cores das equipes
      _team1Color = _parseColor(team1Data['team_color'] as String?);
      _team2Color = _parseColor(team2Data['team_color'] as String?);

      debugPrint(
        "Titulares Padrão Time 1 (${startersHomeIds.length}): $startersHomeIds",
      );
      debugPrint(
        "Titulares Padrão Time 2 (${startersAwayIds.length}): $startersAwayIds",
      );

      // 5. Processa e separa as listas
      _calculateLineups(startersHomeIds, startersAwayIds);
    } catch (e) {
      debugPrint("Erro ao carregar elencos: $e");
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erro ao carregar elencos: ${e.toString()}')),
        );
    } finally {
      if (mounted)
        setState(() {
  ...
  Map<String, dynamic>? _findAndRemovePlayer(
  ...
  // --- LÓGICA DE ESCALAÇÃO (Sem mudanças) ---
  void _calculateLineups(
    List<String> startersHomeIds,
    List<String> startersAwayIds,
  ) {
    void applyFallbackLogic(bool isTeam1) {
      if (isTeam1) {
        final team1Goalkeepers = _team1Players
            .where((p) => p['is_goalkeeper'])
            .toList();
        final team1LinePlayers = _team1Players
            .where((p) => !p['is_goalkeeper'])
            .toList();
        _team1TitularGoalkeeper = team1Goalkeepers.isNotEmpty
            ? team1Goalkeepers.first
            : null;
        ...
        _team1Reserves =
            team1Goalkeepers.skip(1).toList() +
            team1LinePlayers.skip(4).toList();
      } else {
        final team2Goalkeepers = _team2Players
            .where((p) => p['is_goalkeeper'])
            .toList();
        final team2LinePlayers = _team2Players
            .where((p) => !p['is_goalkeeper'])
            .toList();
        _team2TitularGoalkeeper = team2Goalkeepers.isNotEmpty
            ? team2Goalkeepers.first
            : null;
        ...
        _team2Reserves =
            team2Goalkeepers.skip(1).toList() +
            team2LinePlayers.skip(4).toList();
      }
    }
    ...
    if (startersHomeIds.length != 5) {
      debugPrint(
        "TIME 1: Usando Fallback (encontrados ${startersHomeIds.length}/5 titulares)",
      );
      applyFallbackLogic(true);
    } else {
      debugPrint("TIME 1: Usando lógica de Posição");
      final starters1 = _team1Players
          .where((p) => startersHomeIds.contains(p['id']))
          .toList();
      _team1Reserves = _team1Players
          .where((p) => !startersHomeIds.contains(p['id']))
          .toList();
      ...
      List<Map<String, dynamic>> lineStarters1 = starters1
          .where((p) => p['is_goalkeeper'] == false)
          .toList();
      ...
      if (_team1Fixo == null)
        _team1Fixo = _findAndRemovePlayer(lineStarters1, null);
      if (_team1Ala1 == null)
        _team1Ala1 = _findAndRemovePlayer(lineStarters1, null);
      if (_team1Ala2 == null)
        _team1Ala2 = _findAndRemovePlayer(lineStarters1, null);
      if (_team1Pivo == null)
        _team1Pivo = _findAndRemovePlayer(lineStarters1, null);
    }

    // --- TIME 2 ---
    if (startersAwayIds.length != 5) {
      debugPrint(
        "TIME 2: Usando Fallback (encontrados ${startersAwayIds.length}/5 titulares)",
      );
      applyFallbackLogic(false);
    } else {
      debugPrint("TIME 2: Usando lógica de Posição");
      final starters2 = _team2Players
          .where((p) => startersAwayIds.contains(p['id']))
          .toList();
      _team2Reserves = _team2Players
          .where((p) => !startersAwayIds.contains(p['id']))
          .toList();
      ...
      List<Map<String, dynamic>> lineStarters2 = starters2
          .where((p) => p['is_goalkeeper'] == false)
          .toList();
      ...
      if (_team2Fixo == null)
        _team2Fixo = _findAndRemovePlayer(lineStarters2, null);
      if (_team2Ala1 == null)
        _team2Ala1 = _findAndRemovePlayer(lineStarters2, null);
      if (_team2Ala2 == null)
        _team2Ala2 = _findAndRemovePlayer(lineStarters2, null);
      if (_team2Pivo == null)
        _team2Pivo = _findAndRemovePlayer(lineStarters2, null);
    }

    _team1Reserves.sort(
      (a, b) => (a['jersey_number'] ?? 99).compareTo(b['jersey_number'] ?? 99),
    );
    _team2Reserves.sort(
      (a, b) => (a['jersey_number'] ?? 99).compareTo(b['jersey_number'] ?? 99),
    );
  ...
  // --- _showClearSuspensionDialog (Sem mudanças) ---
  ...
  // --- _fetchPlayersForTeam (Sem mudanças) ---
  ...
  // --- _showPlayerSelectionDialog (Sem mudanças) ---
  Future<void> _showPlayerSelectionDialog({
    ...
    selectedPlayer = await showDialog<Map<String, dynamic>>(
      context: context,
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                subtitle: Text(
                  "#${reserve['jersey_number'] ?? '-'} - ${reserve['position'] ?? 'Linha'}",
                ),
    ...
    if (selectedPlayer != null) {
      setState(() {
        ...
        if (isTeam1) {
          switch (positionKey) {
            case 'GK':
              _team1TitularGoalkeeper = selectedPlayer;
              break;
            case 'Fixo':
              _team1Fixo = selectedPlayer;
              break;
            case 'Ala1':
              _team1Ala1 = selectedPlayer;
              break;
            case 'Ala2':
              _team1Ala2 = selectedPlayer;
              break;
            case 'Pivo':
              _team1Pivo = selectedPlayer;
              break;
          }
        } else {
          switch (positionKey) {
            case 'GK':
              _team2TitularGoalkeeper = selectedPlayer;
              break;
            case 'Fixo':
              _team2Fixo = selectedPlayer;
              break;
            case 'Ala1':
              _team2Ala1 = selectedPlayer;
              break;
            case 'Ala2':
              _team2Ala2 = selectedPlayer;
              break;
            case 'Pivo':
              _team2Pivo = selectedPlayer;
              break;
          }
        }
        ...
        _team1Reserves.sort(
          (a, b) =>
              (a['jersey_number'] ?? 99).compareTo(b['jersey_number'] ?? 99),
        );
        _team2Reserves.sort(
          (a, b) =>
              (a['jersey_number'] ?? 99).compareTo(b['jersey_number'] ?? 99),
  ...
  // --- ALTERAÇÃO: _buildPlayerOnCourt AGORA RECEBE E PASSA A COR ---
  ...
  // --- _buildCourtBackground (Sem mudanças) ---
      ...
      return Image.asset(
        ...
        errorBuilder: (context, error, stackTrace) {
          return Container(
            ...
            child: const Center(
              child: Text(
                'Imagem da quadra não encontrada.',
  ...
  // --- _buildMatchDetailsHeader (Sem mudanças) ---
          ...
          children: [
            ...
            Expanded(
              child: _isLoading
                  ...
                  : TabBarView(
                      children: [
                        // --- ABA: QUADRA (Passando as cores) ---
                        _buildCourtView(
                          context,
                          // Time 1
                          _team1TitularGoalkeeper,
                          _team1Fixo,
                          _team1Ala1,
                          _team1Ala2,
                          _team1Pivo,
  ...
  // --- ALTERAÇÃO: _buildCourtView AGORA RECEBE AS CORES ---
  Widget _buildCourtView(
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          // Nomes dos Times (AGORA COM LOGOS MAIORES E MAIS PRÓXIMOS)
          Padding(
            padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 16.0),
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly, // Distribui uniformemente
              children: [
                // Logo e Nome do Time 1
                Column(
                  children: [
                ...
                // Adicione um pequeno espaço entre os logos e os nomes, se quiser
                // const SizedBox(width: 16), 
                // Você pode remover o SizedBox se mainAxisAlignment.spaceEvenly for suficiente
                
                // Logo e Nome do Time 2
                Column(
                  children: [
          ...
          // A Quadra (sem mudança significativa nesta parte)
          Padding(
            ...
            child: AspectRatio(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  final team1FixoPos = Alignment(0.0, -0.53);
                  ...
                  final team2FixoPos = Alignment(0.0, 0.53);
  ...
  // --- ALTERAÇÃO: _buildReservesSection AGORA RECEBE AS CORES ---
  ...
  // --- ALTERAÇÃO: _buildTeamReservesList AGORA RECEBE E PASSA A COR ---
  ...
  // --- Widget _buildDetailsView (Sem mudanças) ---
  ...
  // --- Widget _buildTeamSection (Sem mudanças) ---
  Widget _buildTeamSection(
      ...
      List<Map<String, dynamic>> players) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.center, // Centraliza os elementos da coluna
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.center, // Centraliza o Row (logo e nome)
          children: [
            if (teamShieldUrl.isNotEmpty)
              CachedNetworkImage(
                imageUrl: teamShieldUrl,
              ...
              child: ListTile(
                ...
                onTap: () {
                  if (AdminService.isAdmin && isSuspended) {
                    ...
                    _showClearSuspensionDialog(context, fakeDoc);
                  }
                },
              ),
            );

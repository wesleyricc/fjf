...
class _FreeAgentsMarketScreenState extends State<FreeAgentsMarketScreen> {
  ...
  Widget build(BuildContext context) {
    final portalAuth = Provider.of<PortalAuthService>(context);
    final user = portalAuth.currentPortalUser;
    ...
    if (user == null) {
      return _buildAccessDenied(
        ...
        "Para acessar o mercado de atletas, faça login no Portal do Atleta."
      );
    }
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: _selectedPositionFilter == 'Todos'
                  ? FirebaseFirestore.instance.collection('free_agents').limit(50).snapshots()
                  : FirebaseFirestore.instance.collection('free_agents').where('position', isEqualTo: _selectedPositionFilter).limit(50).snapshots(),
              builder: (context, snapshot) {
                ...
                // Ordenação local porque removemos o orderBy do Firestore
                agents.sort((a, b) => b.createdAt.compareTo(a.createdAt));
  ...
  Widget _buildAccessDenied(BuildContext context, String title, String msg) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Mercado de Atletas"),
        flexibleSpace: Container(
